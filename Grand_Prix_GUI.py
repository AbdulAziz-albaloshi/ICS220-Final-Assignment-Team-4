...
class GrandPrixApp:
    ...
    def ticket_purchase_screen(self):
        ...
        # Sub-function to handle ticket purchase
        def submit_ticket():
            ...
            # Check if price and discount are numeric and non-negative
            try:
                t_price_float = float(t_price)
                t_discount_float = float(t_discount)
                if t_price_float < 0 or t_discount_float < 0:
                    messagebox.showerror("Input Error", "Ticket Price and Discounted Amount must be positive numbers.")
                    return
            except ValueError:
                messagebox.showerror("Input Error", "Ticket Price and Discounted Amount must be numeric values.")
                return

            # Create ticket and associated payment (composition)
            ...
            # Create account, link ticket to account, and link account to fan
